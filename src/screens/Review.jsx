...
//     const requestData = {
//       restaurant_id: 13,
//       order_id: 25, // Replace with the actual order ID
...
const Review = () => {
  ...
  const [reviewText, setReviewText] = useState('');
  const { order_number } = useParams();
  const { restaurantId } = useRestaurantId(); // Fetch restaurantId from context
  const userData = JSON.parse(localStorage.getItem('userData'));
  const customerId = userData ? userData.customer_id : null;
  ...
  }, [restaurantId, order_number, customerId]);
  ...
  const handleSubmitReview = async () => {
    const requestData = {
      restaurant_id: 1, // Use restaurantId from context
      order_number: order_number, // Use order_number from URL params
      customer_id: customerId, // Use customerId from local storage
      rating: rating,
      review: reviewText
    };
